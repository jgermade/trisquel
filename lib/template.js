...
var _ = require('nitro-tools/lib/kit-type'),
    Scope = require('./scope'),
...
function parse(tmpl){
  ...
  tmpl.replace(REmatch,function(match, cmd, expression){
...
template.filter = require('./filter');
...
template.cmd('include', function (scope, expression) {
  // console.log('include', expression, scope.eval(expression), scope);
